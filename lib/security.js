...
// Validates a post-login/signup redirect target so it can only ever point
// somewhere inside this app — never used directly from req.query/req.body
// without going through this first. A value like "https://evil.example/x"
// or the protocol-relative "//evil.example/x" would otherwise send someone
// straight off this domain immediately after a real, successful login,
// which is exactly the kind of thing worth trusting less than it looks —
// the domain in the address bar was genuinely baselineskills.com right up
// until the moment it wasn't. Only a path that starts with a single "/"
// and does not start with "//" (the protocol-relative bypass) is accepted;
...
module.exports = { createIpRateLimiter, createLoginAttemptTracker, requireSameOrigin, clientIp, safeNextPath };
